...
export default function LandingPage() {
  ...
  const SAMPLE_VIDEOS = [
    ...
    { id: 5, title: '성현 & 유진', desc: '온라인에서 만나 3년, 첫 만남 장소에서 프로포즈', mood: t.landing.wedding, bg: 'bg-dark-bg/80', duration: '1:03', moodColor: 'bg-text-secondary' },
    { id: 6, title: '현우 & 다은', desc: '20년 만에 다시 만나 사랑이 된 이야기', mood: t.landing.wedding, bg: 'bg-dark-bg/60', duration: '1:07', moodColor: 'bg-text-secondary' },
  ]
  ...
  return (
    <main className="min-h-screen bg-bg text-text">
        ...
        <div className="max-w-6xl mx-auto px-4 bg-bg/90 backdrop-blur-md border border-border rounded-2xl shadow-sm">
          <div className="h-14 flex items-center justify-between">
            <div className="flex items-center gap-6 sm:gap-8">
              <span className="text-base font-black tracking-tight text-text font-serif">{t.common.brand}</span>
              <nav className="hidden md:flex items-center gap-5 text-sm text-text-secondary">
                <a href="#features" className="hover:text-text transition-colors duration-200 cursor-pointer">{t.landing.navFeatures}</a>
                <a href="#samples" className="hover:text-text transition-colors duration-200 cursor-pointer">{t.landing.navSample}</a>
                <Link href="/blog" className="hover:text-text transition-colors duration-200 cursor-pointer">{t.common.blog}</Link>
                <a href="#pricing" className="hover:text-text transition-colors duration-200 cursor-pointer">{t.landing.navPricing}</a>
              </nav>
            </div>
            <div className="flex items-center gap-2">
              <LangSwitcher />
              <Link href="/login" className="hidden sm:block text-sm text-text-secondary hover:text-text transition-colors duration-200 px-2 cursor-pointer">{t.common.login}</Link>
              <Link href="/create" className="px-4 py-2 bg-primary hover:bg-primary-dark text-white text-sm font-semibold rounded-xl transition-colors duration-200 whitespace-nowrap cursor-pointer">
      ...
      <section className="pt-28 md:pt-36 pb-10 px-5 sm:px-6 text-center max-w-4xl mx-auto">
        <div className="inline-flex items-center gap-2 bg-primary-light/30 border border-primary-light/50 rounded-lg px-3 py-1 text-xs font-bold text-primary-dark mb-6 md:mb-8 tracking-wide uppercase">
          {t.landing.heroLabel}
        </div>

        <h1 className="font-serif text-5xl sm:text-7xl md:text-8xl font-black leading-none tracking-tight text-text mb-5 md:mb-6">
        ...
        <p className="text-base sm:text-lg text-text-secondary max-w-xl mx-auto mb-8 sm:mb-10 leading-relaxed">
          {t.landing.heroDesc}
        </p>

        <div className="flex flex-col sm:flex-row gap-3 justify-center items-center mb-6">
          <Link href="/create" className="w-full sm:w-auto px-8 py-4 bg-primary hover:bg-primary-dark text-white font-bold text-base rounded-2xl transition-colors duration-200 cursor-pointer">
            {t.landing.heroBtn}
          </Link>
          <a href="#samples" className="w-full sm:w-auto px-8 py-4 bg-bg text-text font-semibold text-base rounded-2xl border border-border hover:border-border-hover transition-colors duration-200 cursor-pointer">
        ...
        <div className="w-full aspect-video rounded-3xl bg-dark-bg overflow-hidden relative shadow-2xl shadow-dark-bg/20">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-8">
            <div className="text-center">
              <p className="text-primary-light text-xs font-bold uppercase tracking-widest mb-3">{t.landing.videoAct}</p>
          ...
          <div className="absolute bottom-4 left-6 right-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="ml-auto text-dark-text/50 text-xs">지훈 & 수연</span>
            </div>
      ...
      {/* ── FEATURES ── */}
      <section id="features" className="py-16 sm:py-20 px-5 sm:px-6 bg-bg">
        <div className="max-w-6xl mx-auto">
          <div className="mb-10 sm:mb-14">
            <p className="text-xs font-bold text-primary uppercase tracking-widest mb-3">기능</p>
            <h2 className="font-serif text-3xl sm:text-4xl font-black text-text tracking-tight">Once Upon Us가 특별한 이유</h2>
          </div>

          {/* Row 1: 3 equal cards */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div className="bg-bg border border-border rounded-2xl p-6 hover:border-border-hover hover:shadow-md transition-all duration-200">
              <div className="w-10 h-10 bg-primary-light/30 rounded-xl flex items-center justify-center mb-4">
                <svg className="w-5 h-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z"/>
                </svg>
              </div>
              <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest">01</span>
              <h3 className="text-lg font-black text-text mt-1 mb-2">{t.landing.feat1n1}<br/>{t.landing.feat1n2}</h3>
              <p className="text-text-secondary text-sm leading-relaxed">{t.landing.feat1desc}</p>
              <Link href="/create" className="inline-flex items-center gap-1.5 text-sm font-bold text-primary mt-4 hover:gap-3 transition-all duration-200 cursor-pointer">
                {t.landing.feat1btn}
            ...
            <div className="bg-bg border border-border rounded-2xl p-6 hover:border-border-hover hover:shadow-md transition-all duration-200">
              <div className="w-10 h-10 bg-bg-subtle rounded-xl flex items-center justify-center mb-4">
                <svg className="w-5 h-5 text-text" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/>
                </svg>
              </div>
              <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest">02</span>
              <h3 className="text-lg font-black text-text mt-1 mb-2">{t.landing.feat2n1}<br/>{t.landing.feat2n2}</h3>
            ...
            <div className="bg-bg border border-border rounded-2xl p-6 hover:border-border-hover hover:shadow-md transition-all duration-200">
              ...
              <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest">03</span>
              <h3 className="text-lg font-black text-text mt-1 mb-2">{t.landing.feat3label}</h3>
              <div className="flex items-baseline gap-2 mb-2">
                <span className="text-5xl font-black text-primary leading-none">{t.landing.feat3time}</span>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-bg border border-border rounded-2xl p-6 hover:border-border-hover hover:shadow-md transition-all duration-200">
              ...
              <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest">04</span>
              <h3 className="text-lg font-black text-text mt-1 mb-2">{t.landing.feat4n1}<br/>{t.landing.feat4n2}</h3>
              <p className="text-text-secondary text-sm leading-relaxed">{t.landing.feat4desc}</p>
              <div className="mt-3 flex items-center gap-2">
                <span className="text-xs font-semibold bg-primary-light/30 text-primary px-2.5 py-1 rounded-full">오케스트라 BGM</span>
              </div>
            </div>
            ...
            <div className="bg-bg border border-border rounded-2xl p-6 hover:border-border-hover hover:shadow-md transition-all duration-200">
              ...
              <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest">05</span>
              <h3 className="text-lg font-black text-text mt-1 mb-2">{t.landing.feat5n1}<br/>{t.landing.feat5n2}</h3>
              <p className="text-text-secondary text-sm leading-relaxed">{t.landing.feat5desc}</p>
              <div className="mt-3 flex flex-wrap gap-1.5">
                {['나레이션 편집', '이미지 재생성', '장면 조정'].map(tag => (
                  <span key={tag} className="text-xs font-medium bg-bg-subtle text-text-secondary px-2.5 py-1 rounded-full">{tag}</span>
                ))}
              </div>
            </div>
          </div>
      ...
      <section id="samples" className="py-16 sm:py-20 px-5 sm:px-6 bg-bg-subtle">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-end justify-between mb-8 sm:mb-10">
            ...
            <Link href="/create" className="hidden md:block text-sm font-bold text-primary hover:text-primary-dark transition-colors duration-200 cursor-pointer">
            ...
            {SAMPLE_VIDEOS.map(v => (
              <div key={v.id} className="group cursor-pointer">
                <div className={`w-full aspect-video rounded-xl sm:rounded-2xl ${v.bg} flex items-center justify-center mb-2 sm:mb-3 relative overflow-hidden`}>
                  ...
                  <span className="absolute bottom-2 right-2 bg-dark-bg/50 text-white text-xs px-1.5 sm:px-2 py-0.5 rounded font-medium">{v.duration}</span>
                ...
                <p className="font-bold text-text text-xs sm:text-sm">{v.title}</p>
                <p className="text-text-secondary text-xs mt-0.5 hidden sm:block">{v.desc}</p>
              </div>
            ))}
          </div>
          <div className="mt-6 text-center md:hidden">
            <Link href="/create" className="text-sm font-bold text-primary hover:text-primary-dark transition-colors duration-200 cursor-pointer">
              {t.landing.samplesBtn}
            </Link>
          </div>
        </div>
      ...
      <section className="py-16 sm:py-20 px-5 sm:px-6 bg-bg">
        <div className="max-w-5xl mx-auto">
          ...
          <h2 className="font-serif text-3xl sm:text-4xl font-black text-text mb-10 sm:mb-14">{t.landing.processTitle}</h2>

          {/* Horizontal timeline on desktop, vertical on mobile */}
          <div className="relative">
            {/* Connecting line — desktop */}
            <div className="hidden md:block absolute top-8 left-[calc(12.5%+0px)] right-[calc(12.5%+0px)] h-0.5 bg-gradient-to-r from-primary-light/50 via-primary-light to-border-hover z-0" />
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            ...
            <div className="rounded-2xl border-2 border-primary bg-primary p-6 sm:p-7 relative shadow-lg shadow-primary/20">
              ...
              <p className="text-xs mb-4 text-primary-light/70 relative">{t.checkout.proDesc}</p>
          ...
          <div className="bg-dark-bg rounded-2xl p-6 sm:p-7 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-10">
            ...
            <a href="mailto:hello@onceuponus.kr" className="flex-shrink-0 px-5 py-2.5 bg-bg text-text text-sm font-bold rounded-xl hover:bg-bg-subtle transition-colors duration-200 cursor-pointer">
              {t.landing.b2bBtn}
            </a>
          </div>

          <div className="text-center">
            <Link href="/create" className="inline-block px-10 py-4 bg-primary hover:bg-primary-dark text-white font-bold text-base rounded-2xl transition-colors duration-200 cursor-pointer">
      ...
      <footer className="py-12 px-5 sm:px-6 border-t border-border bg-bg">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center gap-4 mb-8">
            <span className="text-xl font-black text-text">{t.common.brand}</span>
            <p className="text-sm text-text-muted max-w-xs">프로포즈와 결혼식, 두 사람의 이야기를 세상에 하나뿐인 애니메이션으로</p>
          </div>
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6 border-t border-border">
            <div className="flex items-center gap-6 text-sm text-text-muted">
              <Link href="/blog" className="hover:text-text-secondary transition-colors duration-200 cursor-pointer">{t.landing.footerBlog}</Link>
              <a href="mailto:hello@onceuponus.kr" className="hover:text-text-secondary transition-colors duration-200 cursor-pointer">{t.landing.footerContact}</a>
              <a href="#pricing" className="hover:text-text-secondary transition-colors duration-200 cursor-pointer">{t.landing.navPricing}</a>
            </div>
            <p className="text-sm text-text-muted">© 2024 Once Upon Us. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </main>
